...
def get_model():
    ...
    c_key=request.remote_addr+":"+str(data["pid"])
    if c_key not in ALL_CLIENTS:
...
def post_params():
    ...
    if N_CLIENTS >1:
        ...
        if (ALL_PARAMS_CNT) ==1 and COMPLETE:
            U_TIME_STAMP=datetime.now()+timedelta(seconds=2)
            print(f"Waiting time stamap: {str(U_TIME_STAMP)}")
...
if __name__ == "__main__":
    ...
    port_no = 5500
    print(f"Sever is running at {IPAddr}:{port_no}")
    # for listening to any network
    app.run(host="0.0.0.0", debug=False, port=port_no)
